...
import { newScreen , screen} from "$lib/store/screen";
...
$: isSeed = false
$: tabs = 1
const audio = browser && new Audio(background);
...
const handleMinesGameEncrypt = (async()=>{
    is_loading = true
    await axios.get(`${serverUrl()}/api/user/mine-game/mine-encrypt`,{
        headers: {
...
{#if !is_loading}
<div id="dice-main">
    <div id="game-Mines" class={`sc-haTkiu lmWKWf ${$newScreen > 1200 ? "game-style0" : "game-style1" } sc-gDGHff gYWFhf`}>
        <div class="game-area">
            <div class="game-main">
                <Gameview /> 
